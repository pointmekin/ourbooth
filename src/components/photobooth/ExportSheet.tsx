...
import { X, Download, FileImage, Film, Sparkles } from 'lucide-react'
...
export function ExportSheet({ isOpen, onClose }: ExportSheetProps) {
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      a.href = url
      a.download = `ourbooth-${Date.now()}.${exportType === 'gif' ? 'gif' : 'png'}`
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-end md:items-center md:justify-center pointer-events-none">
        <div className="pointer-events-auto w-full md:w-[400px] bg-neutral-950 border border-white/10 rounded-t-3xl md:rounded-2xl p-6 pb-10 md:pb-6 safe-area-inset-bottom md:max-h-[90vh] md:overflow-y-auto animate-in slide-in-from-bottom md:slide-in-from-bottom-0 md:zoom-in-95 duration-300">
          {/* Handle - mobile only */}
